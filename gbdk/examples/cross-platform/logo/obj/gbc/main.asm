...
;src/main.c:7: fill_bkg_rect(0, 0, DEVICE_SCREEN_WIDTH, DEVICE_SCREEN_HEIGHT, 0);
	...
	call	_fill_bkg_rect
	add	sp, #5
;src/main.c:11: if (_cpu == CGB_TYPE) {
	ld	a, (#__cpu)
	sub	a, #0x11
	jr	NZ, 00102$
;src/main.c:12: set_bkg_palette(BKGF_CGB_PAL0, GBDK_2020_logo_PALETTE_COUNT, GBDK_2020_logo_palettes);
...
;../../../include/gb/gb.h:1836: set_bkg_data(first_tile, nb_tiles, data);
	...
	call	_set_bkg_data
	add	sp, #4
;src/main.c:19: if (_cpu == CGB_TYPE) {
	ld	a, (#__cpu)
	sub	a, #0x11
	jr	NZ, 00104$
;src/main.c:20: VBK_REG = VBK_ATTRIBUTES;
	ld	a, #0x01
	ldh	(_VBK_REG + 0), a
;src/main.c:25: GBDK_2020_logo_map_attributes);
;src/main.c:21: set_tile_map((DEVICE_SCREEN_WIDTH - (GBDK_2020_logo_WIDTH >> 3)) >> 1, 
	...
	call	_set_bkg_tiles
	add	sp, #6
;src/main.c:26: VBK_REG = VBK_TILES;
	xor	a, a
	ldh	(_VBK_REG + 0), a
00104$:
